...
@app.route('/api/updateRankRev', methods=['POST'])
def update_rank_rev():
    data = request.json
    nominee_id = data.get('nomineeId')
    rank_rev = data.get('rankRev')

    # Call the method from the Nominee class to update the rank_rev
    Nominee.update_rank_rev(nominee_id, rank_rev)
