...
############################################# DELETE (Returns No DATA) #############################################
    ...
    @classmethod
    def update_rank_rev(cls, nominee_id, rank_rev):
        query = "UPDATE nominees SET rank_rev = %(rank_rev)s WHERE id = %(nominee_id)s;"
        data = {
    ...
    @classmethod
    def get_rank(cls, nominee_id):
        query = "SELECT rank_rev FROM nominees WHERE id = %(id)s;"
        result = connectToMySQL(DATABASE).query_db(query, {'id': nominee_id})
        if result:
            return result[0]['rank_rev']
        return None
